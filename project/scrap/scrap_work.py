...
# def CreateTableau(a, b, c, n, phase_one_optimization):
#     m = len(c)
#     tableau = np.hstack([a, np.eye(n), b[:, None]])
    
#     if phase_one_optimization:
#         mask = b < 0
#         tableau[mask] *= -1
    
#     final_row = np.hstack([-c, np.zeros(n + 1)])
#     tableau = np.vstack([tableau, final_row])

#     phase_one_row = tableau[:-1].sum(axis=0)
#     return tableau, phase_one_row

# def SelectPivotElement(a, phase_one_optimization, phase_one_row):
#     if phase_one_optimization:
#         pivot_col = np.argmax(phase_one_row[:-1])
#     else:
#         pivot_col = np.argmin(a[-1, :-1])

#     valid_rows = a[:-1, pivot_col] > 0
#     ratios = np.where(valid_rows, a[:-1, -1] / a[:-1, pivot_col], np.inf)

#     if np.all(ratios == np.inf):
#         return True, None
    
#     pivot_row = np.argmin(ratios)
#     return False, (pivot_row, pivot_col)

# def ProcessPivotElement(a, pivot_row, pivot_col):
#     a[pivot_row] /= a[pivot_row, pivot_col]
#     a[:, pivot_col] = 0
#     a[pivot_row, pivot_col] = 1
#     a -= np.outer(a[:, pivot_col], a[pivot_row])
...
#     while not np.all(tableau[-1, :-1] >= 0):
#         no_solution, pivot = SelectPivotElement(tableau, False, phase_one_row)
#         if no_solution:
#             return [-1]
#         pivot_row, pivot_col = pivot
#         tableau = ProcessPivotElement(tableau, pivot_row, pivot_col)
...
#     else:
#         print("Bounded solution")
#         print(' '.join(f'{x:.18f}' for x in column))
